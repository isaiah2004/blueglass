 ...
 *
 * IMPORT ORDER IS LOAD-BEARING. Keep this as the first import of any sheet component test,
 * above every other import.
 *
 * The real home for this is a `define` or a setup file in `vitest.config.ts`, which belongs
 * to whoever owns that file. `features/reader/testing/dev-global.ts` is the same three lines
 * for the same reason; when the config gains the define, both go.
 */
